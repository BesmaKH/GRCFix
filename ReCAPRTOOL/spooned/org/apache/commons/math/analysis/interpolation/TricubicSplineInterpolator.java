 ...
 * @since 2.2
 */
public class TricubicSplineInterpolator implements 
org.apache.commons.math.analysis.interpolation.TrivariateRealGridInterpolator {
    /**
     * {@inheritDoc}
     */
    public org.apache.commons.math.analysis.interpolation.TricubicSplineInterpolatingFunction interpolate(final double[] xval, final 
    double[] yval, final 
    double[] zval, final 
    double[][][] fval) throws 
    org.apache.commons.math.MathException {
        ...
        final org.apache.commons.math.analysis.interpolation.BicubicSplineInterpolatingFunction[] xSplineYZ = 
        new org.apache.commons.math.analysis.interpolation.BicubicSplineInterpolatingFunction[xLen];
        for (int i = 0; i < xLen; i++) {
            xSplineYZ[i] = bsi.interpolate(yval, zval, fval[i]);
        }

        // For each line y[j] (0 <= j < yLen), construct a 2D spline in z and x
        final org.apache.commons.math.analysis.interpolation.BicubicSplineInterpolatingFunction[] ySplineZX = 
        new org.apache.commons.math.analysis.interpolation.BicubicSplineInterpolatingFunction[yLen];
        for (int j = 0; j < yLen; j++) {
            ySplineZX[j] = bsi.interpolate(zval, xval, fvalZX[j]);
        }

        // For each line z[k] (0 <= k < zLen), construct a 2D spline in x and y
        final org.apache.commons.math.analysis.interpolation.BicubicSplineInterpolatingFunction[] zSplineXY = 
        new org.apache.commons.math.analysis.interpolation.BicubicSplineInterpolatingFunction[zLen];
        for (int k = 0; k < zLen; k++) {
        ...
        return new org.apache.commons.math.analysis.interpolation.TricubicSplineInterpolatingFunction(xval, yval, zval, fval, 
        dFdX, dFdY, dFdZ, 
