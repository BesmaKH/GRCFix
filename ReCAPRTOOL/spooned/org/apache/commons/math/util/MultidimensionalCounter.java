...
public class MultidimensionalCounter implements java.lang.Iterable<java.lang.Integer> {
    ...
    public class Iterator implements java.util.Iterator<java.lang.Integer> {
         ...
         */         public java.lang.Integer next() {             for (int i = last; i >= 0; i--) {
                if ((counter[i]) == ((size[i]) - 1)) {
                    counter[i] = 0;
                }else {
                    ++(counter[i]);
     ...
     * 		negative or zero.
     */     public MultidimensionalCounter(int... size) {         dimension = size.length;
     ...
     * @return the index within the unidimensionl counter.
     * @throws {@link
     * 		DimensionMismatchException} if the size of {@code c}
     ...
     */     public int getCount(int... c) {         if ((c.length) != (dimension)) {             throw new org.apache.commons.math.exception.DimensionMismatchException(c.length, dimension);
        }
        int count = 0;
        for (int i = 0; i < (dimension); i++) {
            final int index = c[i];
