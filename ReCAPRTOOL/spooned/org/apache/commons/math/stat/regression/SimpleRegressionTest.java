...
public final class SimpleRegressionTest extends junit.framework.TestCase {
    ...
    private double[][] data = new double[][]{ new double[]{ 0.1, 0.2 }, new double[]{ 338.8, 337.4 }, new double[]{ 118.1, 118.2 }, 
    new double[]{ 888.0, 884.6 }, new double[]{ 9.2, 10.1 }, new double[]{ 228.1, 226.5 }, new double[]{ 668.5, 666.3 }, new double[]{ 998.5, 996.3 }, 
    new double[]{ 449.1, 448.6 }, new double[]{ 778.9, 777.0 }, new double[]{ 559.2, 558.2 }, new double[]{ 0.3, 0.4 }, new double[]{ 0.1, 0.6 }, new double[]{ 778.1, 775.5 }, 
    new double[]{ 668.8, 666.9 }, new double[]{ 339.3, 338.0 }, new double[]{ 448.9, 447.5 }, new double[]{ 10.8, 11.6 }, new double[]{ 557.7, 556.0 }, 
    new double[]{ 228.3, 228.1 }, new double[]{ 998.0, 995.8 }, new double[]{ 888.8, 887.6 }, new double[]{ 119.6, 120.2 }, new double[]{ 0.3, 0.3 }, 
    new double[]{ 0.6, 0.3 }, new double[]{ 557.6, 556.8 }, new double[]{ 339.3, 339.1 }, new double[]{ 888.0, 887.2 }, new double[]{ 998.5, 999.0 }, 
    new double[]{ 778.9, 779.0 }, new double[]{ 10.2, 11.1 }, new double[]{ 117.6, 118.3 }, new double[]{ 228.9, 229.2 }, new double[]{ 668.4, 669.1 }, 
    new double[]{ 449.2, 448.9 }, new double[]{ 0.2, 0.5 } };
    ...
    private double[][] corrData = new double[][]{ new double[]{ 101.0, 99.2 }, new double[]{ 100.1, 99.0 }, new double[]{ 100.0, 100.0 }, 
    new double[]{ 90.6, 111.6 }, new double[]{ 86.5, 122.2 }, new double[]{ 89.7, 117.6 }, new double[]{ 90.6, 121.1 }, new double[]{ 82.8, 136.0 }, 
    new double[]{ 70.1, 154.2 }, new double[]{ 65.4, 153.6 }, new double[]{ 61.3, 158.5 }, new double[]{ 62.5, 140.6 }, new double[]{ 63.6, 136.2 }, 
    new double[]{ 52.6, 168.0 }, new double[]{ 59.7, 154.3 }, new double[]{ 59.5, 149.0 }, new double[]{ 61.3, 165.5 } };
    ...
    private double[][] infData = new double[][]{ new double[]{ 15.6, 5.2 }, new double[]{ 26.8, 6.1 }, new double[]{ 37.8, 8.7 }, new double[]{ 36.4, 8.5 }, 
    new double[]{ 35.5, 8.8 }, new double[]{ 18.6, 4.9 }, new double[]{ 15.3, 4.5 }, new double[]{ 7.9, 2.5 }, new double[]{ 0.0, 1.1 } };
    ...
    private double[][] infData2 = new double[][]{ new double[]{ 1, 1 }, new double[]{ 2, 0 }, new double[]{ 3, 5 }, new double[]{ 4, 2 }, 
    new double[]{ 5, -1 }, new double[]{ 6, 12 } };
    ...
    public void testNorris() {
        ...
        junit.framework.Assert.assertEquals("slope std err", 4.29796848199937E-4, 
        regression.getSlopeStdErr(), 1.0E-11);
        junit.framework.Assert.assertEquals("number of observations", 36, regression.getN());
        junit.framework.Assert.assertEquals("intercept", (-0.262323073774029), 
        regression.getIntercept(), 1.0E-11);
        junit.framework.Assert.assertEquals("std err intercept", 0.232818234301152, 
        regression.getInterceptStdErr(), 1.0E-11);
        junit.framework.Assert.assertEquals("r-square", 0.999993745883712, 
        regression.getRSquare(), 1.0E-11);
        junit.framework.Assert.assertEquals("SSR", 4255954.13232369, 
        regression.getRegressionSumSquares(), 1.0E-8);
        junit.framework.Assert.assertEquals("MSE", 0.782864662630069, 
        regression.getMeanSquareError(), 1.0E-9);
        junit.framework.Assert.assertEquals("SSE", 26.6173985294224, 
        regression.getSumSquaredErrors(), 1.0E-8);
        // ------------  End certified data tests
        junit.framework.Assert.
        assertEquals("predict(0)", (-0.262323073774029), 
        regression.predict(0), 1.0E-11);
        junit.framework.Assert.assertEquals("predict(1)", (1.00211681802045 - 0.262323073774029), 
        regression.predict(1), 1.0E-11);
    }
    ...
    public void testInference() throws java.lang.Exception {
        ...
        junit.framework.Assert.assertEquals("slope std err", 0.011448491, 
        regression.getSlopeStdErr(), 1.0E-10);
        junit.framework.Assert.assertEquals("std err intercept", 0.286036932, 
        regression.getInterceptStdErr(), 1.0E-8);
        junit.framework.Assert.assertEquals("significance", 4.596E-7, 
        regression.getSignificance(), 1.0E-8);
        junit.framework.Assert.assertEquals("slope conf interval half-width", 0.0270713794287, 
        regression.getSlopeConfidenceInterval(), 1.0E-8);
        // infData2
        regression = new org.apache.commons.math.stat.regression.SimpleRegression();
        regression.addData(infData2);
        junit.framework.Assert.assertEquals("slope std err", 1.07260253, 
        regression.getSlopeStdErr(), 1.0E-8);
        junit.framework.Assert.assertEquals("std err intercept", 4.17718672, 
        regression.getInterceptStdErr(), 1.0E-8);
        junit.framework.Assert.assertEquals("significance", 0.261829133982, 
        regression.getSignificance(), 1.0E-11);
        junit.framework.Assert.assertEquals("slope conf interval half-width", 2.97802204827, 
        regression.getSlopeConfidenceInterval(), 1.0E-8);
        // ------------- End R-verified tests -------------------------------

        // FIXME: get a real example to test against with alpha = .01
        junit.framework.Assert.assertTrue("tighter means wider", 
    ...
    public void testRandom() throws java.lang.Exception {
        ...
        junit.framework.Assert.assertTrue(((0.0 < (regression.getSignificance())) && 
