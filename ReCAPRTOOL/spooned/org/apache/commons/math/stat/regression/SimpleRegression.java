...
public class SimpleRegression implements java.io.Serializable {
     ...
     * 		the distribution used to compute inference statistics.
     * @since 1.2
     */     public SimpleRegression(org.apache.commons.math.distribution.TDistribution t) {         super();
        setDistribution(t);
    }
     ...
     * @param y
     * 		dependent variable value
     */     public void removeData(double x, double y) {         if ((n) > 0) {
            double dx = x - (xbar);
            double dy = y - (ybar);
            sumXX -= ((dx * dx) * ((double) (n))) / ((n) - 1.0);
            sumYY -= ((dy * dy) * ((double) (n))) / ((n) - 1.0);
    ...
    public double getSlope() {
        if ((n) < 2) {
            return java.lang.Double.NaN;// not enough data
        }
        if ((java.lang.Math.abs(sumXX)) < (10 * (java.lang.Double.MIN_VALUE))) {
            return java.lang.Double.NaN;// not enough variation in x
        }
        return (sumXY) / (sumXX);
    }
    ...
    public double getInterceptStdErr() {
        ...
        ((getMeanSquareError()) * ((1.0 / ((double) (n))) + (((xbar) * (xbar)) / (sumXX)))));
    }
     ...
     */     public double getSlopeConfidenceInterval(double alpha) throws org.apache.commons.math.MathException {
        ...
        (distribution.inverseCumulativeProbability((1.0 - (alpha / 2.0))));
    }
     ...
     */     public double getSignificance() throws org.apache.commons.math.MathException {
        return 2.0 * (1.0 - (distribution.cumulativeProbability(
        ((java.lang.Math.abs(getSlope())) / (getSlopeStdErr())))));
    }
