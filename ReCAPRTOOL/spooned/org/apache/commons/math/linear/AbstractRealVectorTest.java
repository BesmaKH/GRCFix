...
public class AbstractRealVectorTest extends junit.framework.TestCase {
    ...
    private static void assertEquals(double[] d1, double[] d2) {
        ...
        for (int i = 0; i < (d1.length); i++)             junit.framework.Assert.assertEquals(d1[i], d2[i]);
    }
    ...
    public void testMap() throws java.lang.Exception {
        ...
        org.apache.commons.math.linear.RealVector w = v.map(new org.apache.commons.math.analysis.UnivariateRealFunction() {             public double value(double x) {                 return x * x;}});
    ...
    public void testSparseIterator() throws java.lang.Exception {
        ...
        double[] onlyOne = new double[]{ 0.0, 1.0, 0.0 };
        v = new org.apache.commons.math.linear.AbstractRealVectorTest.TestVectorImpl(onlyOne);
        for (java.util.Iterator<org.apache.commons.math.linear.RealVector.Entry> it = v.sparseIterator(); (it.hasNext()) && ((e = it.next()) != null);) {
            junit.framework.Assert.assertEquals(onlyOne[1], e.getValue());
        }
    ...
    public void testClone() throws java.lang.Exception {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            if (i > 4)                 cloneTime += (java.lang.System.nanoTime()) - start;
            start = java.lang.System.nanoTime();
            v = new double[d.length];
            for (int j = 0; j < (v.length); j++)                 v[j] = (d[j]) + 1234.5678;
            if (i > 4)                 setAndAddTime += (java.lang.System.nanoTime()) - start;
        }
    }
}